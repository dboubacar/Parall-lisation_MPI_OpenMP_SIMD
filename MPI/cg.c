...
void spmv_parallel(const struct csr_matrix_t *A, const double *x, double *y,int rank,int nproc){
	int nz_local,n_local,nrow;
	int *colIndex_local,*ptr_local;
	double *values_local,*x_local,*y_local;
	if(rank==MASTER){
		int n_local_last;
		nrow=A->n;
	  ...
	  //values_displs[0] = A->ptr[0];
		...
		//envoie le vecteur x
		MPI_Bcast((double*)x,A->n,MPI_DOUBLE,MASTER,MPI_COMM_WORLD);
		...
		//recoit le vecteur x
		x_local = (double *)malloc(sizeof(double) * nrow);
		MPI_Bcast(x_local,nrow,MPI_DOUBLE,0,MPI_COMM_WORLD);
		y_local = (double *)malloc(sizeof(double) * n_local);
	}
	struct csr_matrix_t *A_rank = malloc(sizeof(*A_rank));
	A_rank->n = n_local;
	A_rank->nz = nz_local;
	A_rank->ptr = ptr_local;
	A_rank->colIndex = colIndex_local;
	A_rank->values = values_local;
	sp_gemv(A_rank,x_local	,y_local);
	//Root rassemble les composants de y
	MPI_Gather(y_local,n_local,MPI_DOUBLE,y,n_local,MPI_DOUBLE,MASTER,MPI_COMM_WORLD);
	free(ptr_local);free(colIndex_local);free(values_local);free(A_rank);
...
int main(int argc, char **argv)
{
	...
	*scratch;	/* workspace for cg_solve() */
	...
	/* solve values == b */
	spmv_parallel(A,b,x,rank,nproc);

	//cg_solve(A, b, x, THRESHOLD, scratch,argc,argv);
	if(rank==MASTER){
		printf("x %f\n",x[0]);
	/* Check result */
	if (safety_check) {
